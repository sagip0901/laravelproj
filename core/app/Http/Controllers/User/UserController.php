...
class UserController extends Controller {
    public function home() {
        $pageTitle         = 'Dashboard';
        $user              = auth()->user();
        $favoriteTemplates = Favorite::where('user_id', $user->id)->with('template')->limit(5)->get();
        $histories         = History::where('user_id', $user->id)->orderBy('id', 'desc')->limit(5)->get();
        return view('Template::user.dashboard', compact('pageTitle', 'user', 'histories', 'favoriteTemplates'));
    }
    ...
    public function subscribePlan($id) {
        ...
        $user           = auth()->user();
        $pendingPayment = $user->deposits()->pending()->count();
        if ($pendingPayment > 0) {
        ...
        $subscription->expired_date = $plan->duration;
        $subscription->save();
    ...
    public function referrals() {
        ...
        $user           = auth()->user();
        $referrals      = User::where('ref_by', $user->id)->searchable(['username', 'email'])->paginate(getPaginate());
        $withdrawMethod = WithdrawMethod::where('status', Status::ENABLE)->get();
        return view('Template::user.referrals', compact('pageTitle', 'referrals', 'user', 'withdrawMethod'));
    }
